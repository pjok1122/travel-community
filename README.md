...
사진을 모아서 한 번에 전송할 경우, 서버의 트래픽이 몰려 일시적인 병목현상이 발생할 수 있다고 판단했습니다. 
...
Ajax를 이용하여 구현했습니다. 유저가 `좋아요`를 클릭하면, 좋아요 또는 취소를 반영하고, 게시물의 `좋아요의 개수`를 응답하여 화면의 일부만을 바꿨습니다.
...
즉, `임시글`과 `게시글`의 테이블을 분리할 계획을 하고 있습니다.
